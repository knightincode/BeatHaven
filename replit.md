...
**Audio Serving**: The `/api/audio/:folder/:filename` route uses disk-based caching in `/tmp/audio-cache/`. On first request, files are tee-streamed from Object Storage to both the response and disk simultaneously. When the client disconnects (e.g., user switches tracks), the Object Storage download is aborted immediately to prevent resource exhaustion from concurrent 302 MB background downloads. Subsequent requests are served directly from disk via `fs.createReadStream` with native range request support — no files are loaded into memory. For non-zero byte range requests (e.g., `bytes=6998-`), the server first checks the disk cache; if not available, it uses `createRangeStream()` to stream from Object Storage with byte-skipping and bounded output — this prevents 416 errors when the browser sends follow-up range requests before the file is fully cached. A lightweight `HEAD` handler returns metadata (Content-Length, Content-Type) without initiating streams, used by the client's pre-flight check. Stream timeout (120s) prevents hung Object Storage downloads. All error responses include the `path` field for diagnostics, and stream error handlers call `res.end()` to prevent hung connections. A storage health endpoint at `/api/health/storage` verifies Object Storage connectivity.
